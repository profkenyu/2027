...
schedule.forEach((s,i)=>{
  choreography.add(motion[i],{approach:[0,1],duration:s.duration*1000,ease:smooth},s.start*1000);
...
function resize(){
  ...
  camera.position.set(camera.aspect<1?260:560,285,camera.aspect<1?2000:1000);camera.lookAt(0,95,-1000);camera.updateProjectionMatrix();renderer.setSize(innerWidth,innerHeight);
}
function render(t){
  seconds=THREE.MathUtils.clamp(t,0,180);
  choreography.seek(seconds*1000);
  const portrait=camera.aspect<1;
  fleet.forEach((ship,i)=>{
    const s=schedule[i],p=motion[i].approach;
    // Parallel trajectories converge to one vanishing point. Reciprocal depth
    // controls apparent scale smoothly without changing a vessel's dimensions.
    const near=camera.position.z-s.z,far=18000+i*2200;
    const depth=1/THREE.MathUtils.lerp(1/far,1/near,p);
    ship.position.set(s.x*(portrait?.5:1),s.y,camera.position.z-depth);
  ...
  title.style.opacity=String(smooth((seconds-150)/15));
  replay.hidden=seconds<175;archive.hidden=seconds<150;
  if(audio){const rise=smooth((seconds-16)/68),fall=1-smooth((seconds-135)/40);audio.gain.gain.setTargetAtTime(playing?rise*fall*.12:0,audio.ctx.currentTime,.6);}
...
try{
  ...
  scene.add(new THREE.HemisphereLight(0xe6e8e5,0x655749,.55));
  ...
  const fill=new THREE.DirectionalLight(0xd3c5b9,.55);fill.position.set(1800,-1100,600);scene.add(fill);
  ...
  window.FIRST_DAWN={snapshot:()=>({seconds,tier,ships:fleet.filter(s=>s.visible).length,total:fleet.length,paused,triangles:renderer.info.render.triangles,calls:renderer.info.render.calls,pixelRatio,independent:!window.TI_WORLD,animation:'anime.js',deployment:motion.map(s=>s.deploy)}),...(params.has('test')?{seek:render,positions:()=>fleet.map(s=>s.position.toArray())}:{})};
  requestAnimationFrame(tick);
