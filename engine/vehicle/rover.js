...
import { makeLink, solveSuspension } from "./mechanics.js";
...
export class Rover {
  ...
  update(dt) {
    ...
    for (let i = 0; i < this.wheels.length; i++) {
      ...
      solveSuspension(w.userData.suspension, w.position.y);
    }
...
function buildRover() {
  ...
  const grid = C.clipmap.grid;
  const segments = grid >= 600 ? 14 : grid >= 450 ? 10 : 6;
  const wheels = [];
  for (const dz of D.axles) {
    for (const dx of [-D.track, D.track]) {
      const lz = -dz;
      ...
      const anchor = new THREE.Vector3(dx * 0.48, -0.055, lz);
      const hub = new THREE.Vector3(dx, restY, lz);
      const upper = makeLink(0.035, 0.34, armour, segments);
      const lower = makeLink(0.027, 0.34, metal, segments);
      const barrel = makeLink(0.035, 0.34, dark, segments);
      const rod = makeLink(0.017, 0.34, metal, segments);
      const joint = new THREE.Mesh(new THREE.CylinderGeometry(0.061, 0.061, 0.075, segments), dark);
      joint.rotation.x = Math.PI / 2;
      const axle = new THREE.Mesh(new THREE.CylinderGeometry(0.065, 0.065, 0.13, segments), metal);
      axle.rotation.z = Math.PI / 2;
      for (const mesh of [upper, lower, barrel, rod, joint, axle]) {
        transferTag(mesh, "wheel", 10133928, true);
        chassis.add(mesh);
      }
      const suspension = {
        anchor, hub, elbow: new THREE.Vector3(), linkLength: 0.34, side,
        upper, lower, barrel, rod, joint, axle,
        damperAnchor: new THREE.Vector3(dx * 0.35, -0.03, lz + 0.085),
        direction: new THREE.Vector3(), barrelEnd: new THREE.Vector3(), rodStart: new THREE.Vector3()
      };
      upper.userData.designRole = "fixed-suspension-link";
      barrel.userData.designRole = "telescopic-damper";
      Object.assign(w.userData, { restY, suspension });
      solveSuspension(suspension, restY);
      wheels.push(w);
    }
  }
