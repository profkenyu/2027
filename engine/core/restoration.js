...
const koreanLabels = (sample, module, sign) => Object.freeze({ sample, module, sign });
...
export const RESTORATION_ITEMS = Object.freeze([
  Object.freeze({
    ...
    ko: koreanLabels("철-니켈 합금", "하부 골격", "금속 반사 특성"),
    color: 14189621,
    form: "plates",
  ...
  Object.freeze({
    ...
    module: "LOAD PATHS",
    sign: "SPECTRAL SPLIT",
    ko: koreanLabels("규산염 세라믹", "하중 전달부", "규산염 흡수대 분리"),
    color: 13219211,
    form: "prisms",
  ...
  Object.freeze({
    ...
    sign: "RAMAN N\u2082 BAND",
    ko: koreanLabels("분자 질소(N\u2082)", "질소계 원료", "N\u2082 라만 산란대"),
    gauge: "N\u2082",
    color: 12175812,
    form: "frost",
  ...
  Object.freeze({
    ...
    ko: koreanLabels("에탄올(C\u2082H\u2085OH)", "유기 화학 원료", "C\u2013O\u00b7O\u2013H 진동 흡수대"),
    gauge: "EtOH",
    color: 16757276,
    form: "crystals",
    grammar: "dual-band"
...
export class Restoration {
  constructor(lander, heightAt = () => 0, sites = []) {
    ...
    renderDotMatrix(this.registrationFixedCode, "04/04", { label: "외피 구조재 4종 고정 완료" });
  ...
  _syncUi(message = "") {
    ...
    this._syncGauges();
    if (this.label) this.label.textContent = message || this._defaultStatusText();
  ...
  _buildRegistrationCells() {
    this.cells.forEach((cell, index) => {
      ...
      sample.textContent = item.ko.sample;
      module.textContent = item.ko.module;
    ...
    this.gaugeElements.forEach((element, index) => {
      ...
      label.textContent = item.ko.module;
      value.textContent = "000%";
      meter.className = "raw-material-track";
      meter.setAttribute("role", "meter");
      meter.setAttribute("aria-label", `${item.gauge} ${item.ko.module}`);
  ...
  setCompletionRegistration(registration = null, timelineProgress = 0) {
    ...
    if (fixed) {
      ...
      if (this.registrationModule) this.registrationModule.textContent = `${item.ko.sign} → ${item.ko.module}`;
    }
  }
  ...
  _begin(v, now, siteIndex) {
    ...
    this._syncUi(`분광 특성 검출 \xB7 ${item.ko.sign}`);
  ...
  update(v, now, active = true) {
    ...
    if (!this.event) {
      ...
      if (this.complete) {
        ...
        if (this.lastDistance <= SCAN_RADIUS && Math.abs(v.speed) <= SCAN_SPEED) {
          ...
          this._syncUi(`시료 채취 거리 유지 \xB7 ${this.items[site.itemIndex].ko.sign}`);
        }
      }
      return;
    }
    ...
    if (!site || this.lastDistance > CANCEL_RADIUS || Math.abs(v.speed) > 0.18) {
      ...
      this.state = "detected";
      this.sample.visible = this.ring.visible = this.particles.visible = false;
      this._syncUi("분광 측정 중단 \xB7 표적에 재접근 필요");
      return;
    }
    this._animate(now);
    if (this.event.committed && this.event.item.kind === "raw") {
    ...
    if (!this.event.committed && ageMs >= SCAN_MS) {
      ...
      if (fixed) {
        ...
        if (this.complete) {
          this._beginFieldSubtraction(now);
          this._syncUi("현장 시료 회수 완료 \xB7 좌표 기록 6건 생성");
        } else {
          this._syncUi(`시료 채취 완료 \xB7 ${this.event.item.ko.sample}`);
        }
      }
    }
    if (ageMs >= EVENT_MS) {
      const module = this.event.item.ko.module;
