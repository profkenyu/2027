...
for(const passage of [1,2])for(const aspect of [1.6,390/844,844/390]){
 ...
 for(let i=0;i<3;i++){
  ...
  if(passage===2||i===1){const reference=speed((a+b)/2);assert(Math.abs(speed(a+.01)-reference)<.001);assert(Math.abs(speed(b-.01)-reference)<.001);}
 }
 for(const cut of cuts){
  ...
  assert.equal(passageEdit(cut,cuts).veil,1);assert.equal(passageEdit(cut-.1,cuts).veil,0);assert.equal(passageEdit(cut+.1,cuts).veil,0);
 }
}
console.log('PASS both passages: nonzero relative speed at shot boundaries / constant tracking speed / uninterrupted ship velocity / cut cover under 0.2s');
