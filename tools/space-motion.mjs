...
import {pose,displacement,bridgeRadius} from '../works/space/scene.js';
...
for(const passage of [1,2])for(const aspect of [1.6,390/844,844/390]){
 ...
 for(const cut of cuts){
  ...
  for(const edge of [cut-bridgeRadius(passage,cut),cut+bridgeRadius(passage,cut)]){
   ...
   const h=.002;
   const acceleration=t=>relative(t+h).add(relative(t-h)).addScaledVector(relative(t),-2).divideScalar(h*h);
   assert(acceleration(edge-.004).distanceTo(acceleration(edge+.004))<.2,'Camera bridge acceleration jump');
  }
 }
 const expected=passage===2?Math.hypot(12,95)/20*.5:Math.hypot(aspect<1?5:16,8,88)/24*.5;
 for(const t of [51,55,60])assert(Math.abs(speed(t)-expected)<.001,'Post-reveal tracking must return to the previous half-rate');
}
