...
try{
 ...
 await page.screenshot({path:`${out}/planet-03-exploration.png`});await page.keyboard.press('Equal');await page.waitForURL('**/migration.html?**',{timeout:45000});await page.waitForFunction(()=>window.FIRST_DAWN);assert.equal(await page.evaluate(()=>FIRST_DAWN.snapshot().total),6);await page.locator('#sound').click();
 await page.waitForURL('**/arrival.html?**',{timeout:110000});await page.waitForFunction(()=>window.FIRST_DAWN);await page.locator('#sound').click();
 await page.waitForFunction(()=>FIRST_DAWN.snapshot().seconds>=88,null,{timeout:45000});await page.screenshot({path:`${out}/ending-arrival.png`});
 await page.waitForURL('**/ending.html?**',{timeout:100000});await page.waitForFunction(()=>window.BTK_ENDING);await page.waitForFunction(()=>BTK_ENDING.snapshot().seconds>=18,null,{timeout:35000});assert.equal(await page.locator('#line').innerText(),'우리는 이제 여기서 시작한다');await page.screenshot({path:`${out}/ending-caption.png`});report.events.push({ending:await page.evaluate(()=>BTK_ENDING.snapshot())});
