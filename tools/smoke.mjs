...
if (MOBILE) {
  ...
  await page.waitForFunction(() => {
    ...
    return state?.current === "lander" && state.annotationCount === 4 && state.scanDirection === -1 && state.scan < 0.9;
  }, null, { timeout: 18e3 });
  ...
  const scanAfter = await page.evaluate(() => window.TI_BLUEPRINT?.() ?? null);
  blueprintLayout.motionVerified = scanAfter?.current === "lander" &&
    scanAfter.scanDirection === -1 && blueprintLayout.scan - scanAfter.scan > 0.01;
}
