...
try{
 ...
 const open=await browser.newPage({viewport:{width:390,height:844}});await open.goto(`${server.url}/index.html?test`);await open.waitForFunction(()=>window.TI_OPENING_TEST&&window.TI_BLUEPRINT?.().models?.ship);await open.evaluate(()=>TI_OPENING_TEST.seek(1e6));await open.waitForTimeout(2000);assert.equal(await open.locator('#ti-prologue h1').innerText(),'BEYOND THE KNOWN - A Terrafoming Project');assert.equal(await open.evaluate(()=>getComputedStyle(document.querySelector('#ti-prologue h1')).textTransform),'none');await open.screenshot({path:'output/qa/space-01/opening-mobile.png'});await open.close();
