...
for (const test of cases) {
  ...
  const fixedPass = fixed.state?.fixed && fixed.state?.registered === 4 && fixed.state?.phase === "계획 04 → 확인 04" && fixed.state?.sample === "착륙선 외피 고정" && fixed.state?.resources?.length === 2 && fixed.state.resources.every((resource) => resource.value >= 0.999);
