...
try{
  ...
  const data=await page.evaluate(async()=>{
    const {OpeningBlueprintSequence,extractShip}=await import('../works/terra_incognita/opening-blueprints.js');
    const {createFleet}=await import('../works/first_dawn/ships.js');
    const ship=createFleet('high',1)[0];
