...
const REFRESH_NOTO = process.argv.includes("--refresh-noto");
...
async function localNotoFace(text) {
  ...
  if (subsetText !== text) {
    if (!REFRESH_NOTO)
      throw new Error("Noto Sans KR subset is stale: run npm run fonts -- --refresh-noto");
    const [face] = await faces("family=Noto+Sans+KR:wght@400&display=swap", text);
    if (!face?.data)
      throw new Error("Noto Sans KR subset refresh returned no font data");
    await writeFile(NOTO_SUBSET_TEXT, `${text}\n`);
    await writeFile(NOTO_SUBSET_FONT, face.data);
  }
