...
export class AnimeRituals {
  ...
  _renderCompletion() {
    ...
    let interval = "640 ms";
    if (this.completion.cadence < 1) interval = "120 ms";
    else if (this.completion.cadence < 2) interval = "280 ms";
    this.readout.textContent = `원격계 상태 유지 \xB7 다음 시료 ${interval}`;
  }
  _finishCompletion() {
    if (this.readout) this.readout.textContent = "원격계 상태 유지 \xB7 통신 대기";
  }
}
