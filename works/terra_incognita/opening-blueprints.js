...
const ROVER_CALLOUTS = Object.freeze([
  ...
  { name: "8-WHEEL LOAD PATH", detail: "FIXED LINKS + TELESCOPIC DAMPERS" },
...
const LANDER_CALLOUTS = Object.freeze([
  ...
  { name: "LOAD PATHS", detail: "6 JOINTED LEGS + TERRAIN PADS" },
...
export class OpeningBlueprintSequence {
  ...
  _apply(elapsed, force = false) {
    ...
    const motion = reciprocatingScan(local, duration);
  ...
  _present(current) {
    ...
    const rows = rover ? [
      ...
      ["Contact", "8 wheels / fixed-link suspension"],
      ["Power", "6 \xD7 12 photovoltaic matrix"],
      ["Optics", "Mast / 8 mm lens profile"],
...
function extractLander(lander, limit) {
  // Read the same posed meshes as the renderer, not restoration's cached wire.
  // Hidden (unrestored) parts are intentionally included without changing them.
  lander.group.updateMatrixWorld(true);
  const inverse = lander.group.matrixWorld.clone().invert();
  const coords = [], parts = [];
  const bounds = new THREE.Box3().makeEmpty();
  let sourceSegments = 0, meshes = 0;
  for (const part of lander.parts) {
    for (const object of part.objects) {
      ...
      const position = edges.getAttribute("position");
      const count = Math.floor((position?.count ?? 0) / 2);
      sourceSegments += count;
      meshes++;
      const step = Math.max(1, Math.ceil(count / 260));
      for (let segment = 0; segment < count; segment += step) {
        _a.fromBufferAttribute(position, segment * 2).applyMatrix4(matrix);
        _b.fromBufferAttribute(position, segment * 2 + 1).applyMatrix4(matrix);
        coords.push(_a.x, _a.y, _a.z, _b.x, _b.y, _b.z);
        parts.push(part.assembly ?? 0);
      }
      edges.dispose();
    }
  }
...
function reciprocatingScan(local, duration) {
