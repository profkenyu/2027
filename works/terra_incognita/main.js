...
const FIELD_ARCHIVE_STATIONS = Object.freeze({
  terra: Object.freeze(TERRA_RESOURCE_SITES.filter((site) => site.variant < 2).map((site, order) => Object.freeze({
    ...
    planet: "PLANET 01",
    world: "SHEAR WORLD",
    label: `${["철-니켈 합금 시료", "규산염 세라믹 시료", "탄소 복합재 시료", "전도성 격자 시료", "분자 질소(N₂) 서리층", "에탄올 결정상"][site.item]} · 조사 지점 ${site.variant + 1}`,
    x: site.x,
    z: site.z,
    radius: 13,
    order,
...
async function frame() {
  ...
  const restorationHold = world === "terra" && (restoration.holding(now) || restoration.shouldHold(rover.pos));
  const waterHold = world === "desert" && waterMission.shouldHold(rover.pos);
  const geologicalHold = world === "granite" && geologicalMemory.shouldHold(rover.pos);
  ...
  if (world === "terra" && restoration.event && !restoration.event.all && restoration.event.t0 !== resourceSignalAt) {
    ...
    captions.force({
      r: 0,
      ko: `분광 특성 검출 · ${restoration.event.item.ko.sample}`,
...
async function returnToStart() {
  ...
  document.getElementById('ti-dawn-title').style.opacity='0';
