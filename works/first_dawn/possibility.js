...
export function createPossibility(scene,heightAt,evidence,seed){
 ...
 function update(t,camera){
  ...
  for(let i=0;i<model.field.length;i++){
   const o=i*4,concentration=model.field[i],edge=THREE.MathUtils.smoothstep(concentration,.025,.14);
   pixels[o]=69;pixels[o+1]=83;pixels[o+2]=54;pixels[o+3]=Math.round(edge*reveal*205);
  }
