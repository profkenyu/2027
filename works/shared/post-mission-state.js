import {sampleSignatures} from '../first_dawn/mission-response.js';
...
export function readPostMission(){
 try{const s=JSON.parse(sessionStorage.getItem(POST_KEY)||'null');
  ...
  const signatures=s.evidence?.sampleSignatures;
  if(signatures!=null&&(!Array.isArray(signatures)||signatures.length!==6||signatures.some(v=>!Number.isFinite(v)||v<0||v>1)))return null;
  return s;
 }catch{return null;}
}
...
export function beginPostMission(seed,ledger,environment,geology){
 ...
 const state={version:1,phase:'migration',elapsed:0,seed,evidence:{samples:6,water:true,nodes:3,
  ...
  distance:Math.max(0,ledger.journey?.distance??0),sampleSignatures:sampleSignatures(ledger.samples)},outcome:null};savePostMission(state);return state;
}
